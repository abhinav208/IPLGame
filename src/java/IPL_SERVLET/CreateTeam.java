...
public class CreateTeam extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            String teamName = request.getParameter("teamName").trim().toUpperCase();
            TeamBean teamBean = new TeamBean();
            teamBean.setTeamName(teamName);
            TeamLogic teamLogic = new TeamLogic(teamBean);
            String displayMessage = teamLogic.saveTeamDetails();
            HttpSession session=request.getSession();  
            session.setAttribute("displayMessage", displayMessage);
            RequestDispatcher dispatcher = request.getRequestDispatcher("admin_create_team.jsp");
