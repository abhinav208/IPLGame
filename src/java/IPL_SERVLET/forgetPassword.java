...
public class forgetPassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            String userID = request.getParameter("forgetID").trim();
            String displayMessage = "";
            UserBean userBean = new UserBean();
            userBean.setUserName(userID);
            UserLogic userLogic = new UserLogic(userBean);
            userBean = userLogic.getUserDetailByUsernameOrEmail();
            if(userBean.getUserID() != null) {
            MailFunction mailfunction = new MailFunction();
            mailfunction.sendPasswordMail(userBean);
            displayMessage = "Your Password is send to your mail id " + userBean.getUserEmail();
            } else {
                displayMessage = "Invalid UserName or Email";
            }
